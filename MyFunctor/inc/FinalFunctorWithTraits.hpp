...
{
public:
    template<typename F, typename = std::enable_if_t<std::is_invocable_r_v<R, F, Args...>>>
    Functor(F func) :
        ...
        {
            //static_assert(std::is_invocable_r_v<R, F, Args...>, "Functor class can only be combined to a callable object!");
        }
